...
# # ======================================
# st.set_page_config(page_title="Audit Dashboard Aktivitas Pro", layout="wide")

# # Kredensial Login
# USERNAME_TARGET = "useraudit"
# PASSWORD_TARGET = "user123"

# # Injeksi CSS untuk UI yang lebih bersih
# st.markdown("""
#     <style>
#     .block-container { padding-left: 1rem; padding-right: 1rem; padding-top: 2rem; }
#     .stFileUploader { border: 2px dashed #90CAF9; padding: 15px; border-radius: 10px; background-color: #f7fbff; }
#     [data-testid="stMetricValue"] { font-size: 28px; color: #EF4444; }
#     </style>
...
# # FUNGSI GENERATE PDF (VERSI FORMAL UNTUK ATASAN)
# def create_pdf(df_detail, s_date, e_date):
#     pdf = FPDF(orientation='L', unit='mm', format='A4')
#     pdf.add_page()
    
#     # --- STYLE CONFIG ---
#     PRIMARY_COLOR = (31, 73, 125) # Midnight Blue (Formal)
#     SECONDARY_COLOR = (242, 242, 242) # Light Gray
#     TEXT_COLOR = (40, 40, 40)
...
#     pdf.cell(0, 15, "LAPORAN HASIL AUDIT AKTIVITAS KARYAWAN", 0, 1, 'C')
#     pdf.set_font("Arial", 'I', 11)
#     pdf.cell(0, 5, "Sistem Audit Jam Kerja Otomatis", 0, 1, 'C')
...
#     pdf.cell(90, 20, f"  Total Pelanggaran Aktivitas : {total_kasus} Kasus", 1, 0, 'L', True)
#     pdf.cell(10, 20, "", 0, 0)
#     pdf.cell(90, 20, f"  Total Karyawan Terdampak    : {total_staff} Orang", 1, 1, 'L', True)
#     pdf.ln(10)
...
#     pdf.cell(40, 10, "Jumlah Kasus", 1, 1, 'C', True)
    
#     pdf.set_text_color(*TEXT_COLOR)
#     pdf.set_font("Arial", '', 10)
#     for i, (idx, row) in enumerate(df_rank.iterrows(), 1):
#         pdf.cell(20, 8, str(i), 1, 0, 'C')
#         pdf.cell(150, 8, f" {row['AuthName']}", 1, 0, 'L')
#         pdf.cell(40, 8, str(row['Total']), 1, 1, 'C')
...
#     pdf.set_font("Arial", 'B', 14)
#     pdf.set_text_color(*PRIMARY_COLOR)
#     pdf.cell(0, 10, "III. DETAIL TEMUAN AKTIVITAS", 0, 1, 'L')
#     pdf.ln(5)
    
#     pdf.set_fill_color(*PRIMARY_COLOR)
#     pdf.set_text_color(255, 255, 255)
#     pdf.set_font("Arial", 'B', 9)
#     cols = [10, 25, 60, 25, 25, 30, 102]
#     headers = ["No", "Tanggal", "Nama Karyawan", "Jam IN", "Jam OUT", "Waktu Aktv", "Tipe Aktivitas"]
...
#     for i, row in enumerate(df_detail.iterrows(), 1):
#         fill = True if i % 2 == 0 else False
#         pdf.set_fill_color(250, 250, 250)
#         if pdf.get_y() > 180:
#             pdf.add_page()
#             # Redraw headers
#             pdf.set_fill_color(*PRIMARY_COLOR); pdf.set_text_color(255, 255, 255); pdf.set_font("Arial", 'B', 9)
#             for j in range(len(headers)): pdf.cell(cols[j], 10, headers[j], 1, 0, 'C', True)
#             pdf.ln(); pdf.set_text_color(*TEXT_COLOR); pdf.set_font("Arial", '', 8)

#         pdf.cell(cols[0], 8, str(i), 1, 0, 'C', fill)
#         pdf.cell(cols[1], 8, str(row[1]['TRXDATE']), 1, 0, 'C', fill)
#         pdf.cell(cols[2], 8, f" {str(row[1]['AuthName'])[:35]}", 1, 0, 'L', fill)
#         pdf.cell(cols[3], 8, str(row[1]['IN']), 1, 0, 'C', fill)
#         pdf.cell(cols[4], 8, str(row[1]['OUT']), 1, 0, 'C', fill)
#         pdf.cell(cols[5], 8, str(row[1]['CreateTim']), 1, 0, 'C', fill)
#         pdf.cell(cols[6], 8, f" {str(row[1]['Sumber Aktivitas'])}", 1, 1, 'L', fill)
        
#     pdf.ln(10)
#     pdf.set_font("Arial", 'I', 8)
#     pdf.cell(0, 10, f"Dihasilkan otomatis pada: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", 0, 1, 'R')
#     return pdf.output(dest='S').encode('latin-1')

# # ======================================
# # 3. LOGIN SYSTEM WITH ANIMATION
# # ======================================

# def login_form():
#     col_l, col_center, col_r = st.columns([1, 1.2, 1]) 
#     with col_center:
#         st.markdown("<h2 style='text-align: center; color: #2c3e50;'>Sistem Audit Jam Kerja</h2>", unsafe_allow_html=True)
...
#                 if submitted:
#                     if user_input == USERNAME_TARGET and pass_input == PASSWORD_TARGET:
#                         # --- INOVASI ANIMASI LOGIN ---
#                         with st.status("Mengautentikasi Kredensial...", expanded=True) as status:
#                             st.write("🔍 Memverifikasi basis data user...")
#                             time.sleep(0.6)
#                             st.write("🔐 Mengenkripsi koneksi sesi...")
#                             time.sleep(0.6)
#                             st.write("🚀 Mempersiapkan Dashboard Audit...")
#                             time.sleep(0.4)
#                             status.update(label="Login Berhasil!", state="complete", expanded=False)
...
#     st.title("📊 Dashboard Audit Aktivitas")
#     st.markdown("---")
...
#             df_absen.columns = df_absen.columns.str.strip()
...
#             today = date.today()
#             start_date, end_date = st.date_input("📅 Pilih Periode Audit", value=(today - timedelta(days=30), today))
...
#                         df_merge['Trx_T'] = df_merge.apply(lambda r: datetime.datetime.combine(r['TRXDATE'], r['CreateTim']), axis=1)

#                         df_v = df_merge[(df_merge["Trx_T"] < df_merge["Start_S"]) | (df_merge["Trx_T"] > df_merge["End_S"])].copy()
#                         if not df_v.empty:
#                             df_v["Sumber Aktivitas"] = sheet
#                             hasil.append(df_v[["AuthName", "TRXDATE", "IN", "OUT", "CreateTim", "Sumber Aktivitas"]])
...
#                 m_col1.metric("Total Pelanggaran", f"{len(final_df)} Kasus")
#                 m_col2.metric("Personel Terlibat", f"{final_df['AuthName'].nunique()} Orang")
#                 m_col3.metric("Tipe Terbanyak", final_df['Sumber Aktivitas'].mode()[0])
...
#                     x=alt.X('Jam:O', title='Jam Kejadian (24H)'),
#                     y=alt.Y('Hari:O', title='Hari', sort=order_days),
#                     color=alt.Color('count():Q', scale=alt.Scale(scheme='reds'), title='Jumlah Kasus'),
...
#                     df_ranking.insert(0, 'Peringkat', range(1, 1 + len(df_ranking)))
...
#                         y=alt.Y('AuthName', sort='-x', title="Nama"), 
#                         x=alt.X('Total Pelanggaran', title="Jumlah Pelanggaran"),
#                         tooltip=['AuthName', 'Total Pelanggaran']
#                     ).properties(height=300)
#                     st.altair_chart(chart, use_container_width=True)

#                 st.markdown("---")
#                 st.subheader(f"🚨 Detail {len(final_df)} Temuan Aktivitas")
#                 search = st.text_input("🔎 Cari Nama Karyawan...", key="search").upper()
#                 df_display = final_df[final_df["AuthName"].str.contains(search)] if search else final_df
                
#                 st.dataframe(
#                     df_display.style.apply(highlight_violation, axis=0), 
#                     use_container_width=True, hide_index=True,
#                     column_config={
#                         "TRXDATE": "Tanggal", "AuthName": "Nama Karyawan",
#                         "IN": "Jam IN", "OUT": "Jam OUT",
#                         "CreateTim": "Waktu Aktivitas", "Sumber Aktivitas": "Tipe Aktivitas"
#                     }
#                 )
...
#                 st.download_button(
#                     label="📄 Download Laporan Audit Formal (PDF)", 
#                     data=pdf_bytes, 
#                     file_name=f"Official_Audit_Report_{start_date}.pdf", 
#                     mime="application/pdf",
#                     use_container_width=True
#                 )
                
#             else:
#                 st.success("✅ Tidak ditemukan aktivitas mencurigakan pada periode ini.")
#         except Exception as e:
#             st.error(f"❌ Terjadi kesalahan: {e}")
#     else:
#         st.info("⬆️ Silakan unggah file Excel untuk memulai audit.")
    ...
    if uploaded_file:
        ...
        if isinstance(dates, tuple) and len(dates) == 2:
            ...
            if hasil:
                ...
                st.dataframe(final_df[["TRXDATE", "AuthName", "IN", "OUT", "CreateTim", "Selisih_Waktu", "Sumber Aktivitas"]].style.apply(highlight_violation, axis=0), use_container_width=True, hide_index=True)
